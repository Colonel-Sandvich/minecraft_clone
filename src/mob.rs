use crate::player::{
    cam::MouseState,
    classic_controller::classic_move,
    fly_controller::{fly_move, Flying},
};
...
fn update_grounded(
    ...
    mut query: Query<
        ...
        (With<CharacterController>, Without<Flying>),
    >,
) {
...
fn apply_gravity(
    time: Res<Time>,
    mut controllers: Query<(&ControllerGravity, &mut LinearVelocity), Without<Flying>>,
) {
...
fn apply_movement_damping(
    mut query: Query<(&MovementDampingFactor, &mut LinearVelocity, Has<Flying>)>,
) {
    for (damping_factor, mut linear_velocity, is_flying) in &mut query {
        linear_velocity.0.x *= damping_factor.0;
        linear_velocity.0.z *= damping_factor.0;
        if is_flying {
            linear_velocity.0.y *= damping_factor.0;
        }
    }
}
